...
CoinTossDistribution = Distribution[CoinToss]
DiceRollDistribution = Distribution[DiceRoll]

@pytest.mark.unit
class TestCoinTossDistribution:
    @pytest.fixture
    def coin_one(self) -> CoinTossDistribution:
        return CoinTossDistribution([CoinToss.HEADS, CoinToss.TAILS])
    
    @pytest.fixture
    def coin_two(self) -> CoinTossDistribution:
        return CoinTossDistribution(
            outcomes=[CoinToss.TAILS, CoinToss.HEADS],
            weights=[1, 1])

    @pytest.fixture
    def coin_biased(self) -> CoinTossDistribution:
        return CoinTossDistribution.from_pdf(
            {CoinToss.HEADS: 0.9, CoinToss.TAILS: 0.1})
    
    # ---------------------
    # Distribution.outcomes
    # ---------------------
    def test_coin_one_outcomes(self, coin_one: CoinTossDistribution):
        assert coin_one.outcomes == {CoinToss.HEADS, CoinToss.TAILS}
    
    def test_coin_two_outcomes(self, coin_two: CoinTossDistribution):
        assert coin_two.outcomes == {CoinToss.HEADS, CoinToss.TAILS}
    
    def test_coin_biased_outcomes(self, coin_biased: CoinTossDistribution):
        assert coin_biased.outcomes == {CoinToss.HEADS, CoinToss.TAILS}

    # --------------------------
    # Distribution.probabilities
    # --------------------------
    def test_coin_one_probabilities(self, coin_one: CoinTossDistribution):
        assert coin_one.probabilities == (0.5, 0.5)
    
    def test_coin_two_probabilities(self, coin_two: CoinTossDistribution):
        assert coin_two.probabilities == (0.5, 0.5)
    
    def test_coin_biased_probabilities(self, coin_biased: CoinTossDistribution):
    ...
    def test_coin_one_remove_heads(
        self,
        coin_one: CoinTossDistribution
    ):
        coin_one.remove([CoinToss.HEADS])
        assert coin_one == CoinTossDistribution([CoinToss.TAILS])
    ...
    def test_coin_one_bias(
        self,
        coin_one: CoinTossDistribution,
        coin_biased: CoinTossDistribution
    ):
    ...
    def test_coin_one_and_two_intersect(
        ...
        assert coin_one.intersect(coin_two) == Distribution[tuple[CoinToss, CoinToss]]([
    ...
    def test_coin_one_and_biased_intersect(
        ...
        assert (
            ...
            Distribution[tuple[CoinToss, CoinToss]].from_pdf({
    ...
    def test_coin_one_intersect_self_once(
        ...
        truth_coin = Distribution[tuple[CoinToss]](
    ...
    def test_coin_one_intersect_self_twice(
        ...
        truth_coin = Distribution[tuple[CoinToss, CoinToss]].from_pdf({
    ...
    # Distribution.sample()
    # ---------------------
    def test_coin_one_sample_all(self, coin_one: CoinTossDistribution):
        assert set(coin_one.sample(n=2)) == coin_one.outcomes
    
    def test_coin_two_sample_all(self, coin_two: CoinTossDistribution):
        assert set(coin_two.sample(n=2)) == coin_two.outcomes
    
    def test_coin_biased_sample_all(self, coin_biased: CoinTossDistribution):
        assert set(coin_biased.sample(n=2)) == coin_biased.outcomes
    
    # ----------------------------------
    # Distribution.sample_distribution()
    # ----------------------------------
    def test_coin_one_sample_one_distribution(
        self,
        coin_one: CoinTossDistribution
    ):
        coin_one_sampled = coin_one.sample_distribution(1)
        truth_coin = Distribution[tuple[CoinToss]](
            outcomes=[(CoinToss.HEADS,), (CoinToss.TAILS,)])
        assert coin_one_sampled == truth_coin
    
    def test_coin_two_sample_one_distribution(
        self,
        coin_two: CoinTossDistribution
    ):
        coin_two_sampled = coin_two.sample_distribution(1)
        truth_coin = Distribution[tuple[CoinToss]](
            outcomes=[(CoinToss.HEADS,), (CoinToss.TAILS,)])
        assert coin_two_sampled == truth_coin

    def test_coin_biased_sample_one_distribution(
        self,
        coin_biased: CoinTossDistribution
    ):
        coin_two_sampled = coin_biased.sample_distribution(1)
        truth_coin = Distribution[tuple[CoinToss]](
            outcomes=[(CoinToss.HEADS,), (CoinToss.TAILS,)],
            weights=[9, 1]
        )
    ...
    def test_coin_one_sample_two_distributions(
        self,
        coin_one: CoinTossDistribution
    ):
        coin_one_sampled = coin_one.sample_distribution(2, replace=True)
        truth_coin = Distribution[tuple[CoinToss, CoinToss]].from_pdf({
            (CoinToss.HEADS, CoinToss.HEADS): 0.25,
            (CoinToss.HEADS, CoinToss.TAILS): 0.25,
            (CoinToss.TAILS, CoinToss.HEADS): 0.25,
    ...
    def test_coin_two_sample_two_distributions(
        self,
        coin_two: CoinTossDistribution
    ):
        coin_two_sampled = coin_two.sample_distribution(n=2, replace=True)
        truth_coin = Distribution[tuple[CoinToss, CoinToss]].from_pdf({
            (CoinToss.HEADS, CoinToss.HEADS): 0.25,
            (CoinToss.HEADS, CoinToss.TAILS): 0.25,
            (CoinToss.TAILS, CoinToss.HEADS): 0.25,
    ...
    def test_coin_biased_sample_two_distributions_without_replacement(
        self,
        coin_biased: CoinTossDistribution
    ):
        coin_biased_sampled = coin_biased.sample_distribution(n=2, replace=False)
        print(coin_biased_sampled._pdf)
        assert coin_biased_sampled == Distribution[tuple[CoinToss, CoinToss]].from_pdf({
    ...
    def test_coin_explicitly_sorted(self):
        coin_flips = Distribution[tuple[CoinToss, CoinToss]]([
            (CoinToss.HEADS, CoinToss.TAILS),
            (CoinToss.TAILS, CoinToss.HEADS),
            (CoinToss.TAILS, CoinToss.TAILS)
        ])
    ...
    def test_coin_one_intersect_and_sort(
        ...
        assert (
            ...
            Distribution[tuple[CoinToss, CoinToss, CoinToss]].from_pdf({
    ...
    def test_coin_two_sample_without_replacement_and_sort(
        self,
        coin_two: CoinTossDistribution):
        coin_sampled = coin_two.sample_distribution(n=2, replace=False)
        assert (
            coin_sampled.sorted() ==  # pyright: ignore[reportAttributeAccessIssue]
            Distribution[tuple[CoinToss, CoinToss]]([(CoinToss.HEADS, CoinToss.TAILS)])
        )

    # -------------------------
    # Distribution.to_counter()
    # -------------------------
    def test_coin_explicitly_countered(self):
        ...
        assert (
            ...
            Distribution[HashableCounter[CoinToss]].from_pdf({
    ...
    def test_coin_one_sample_without_replacement_and_counter(
        self,
        coin_one: CoinTossDistribution):
        coin_sampled = coin_one.sample_distribution(n=2, replace=False)
        assert (
            coin_sampled.to_counter() == 
            Distribution[HashableCounter[CoinToss]]([
    ...
    def test_coin_one_filter_for_heads(
        ...
        assert (
            ...
            CoinTossDistribution([CoinToss.HEADS])
        )
    ...
    def test_coin_two_filter_for_tails(
        ...
        assert (
            ...
            CoinTossDistribution([CoinToss.TAILS])
        )
    ...
    # ------------------------------------
    # Distribution.filter_by_probability()
    # ------------------------------------
...
@pytest.mark.unit
class TestDiceRollDistribution:
    @pytest.fixture
    def die_one(self) -> DiceRollDistribution:
        return DiceRollDistribution([DiceRoll.ONE, DiceRoll.TWO, DiceRoll.THREE,
                              DiceRoll.FOUR, DiceRoll.FIVE, DiceRoll.SIX])
    
    @pytest.fixture
    def die_two(self) -> DiceRollDistribution:
    ...
    @pytest.fixture
    def die_biased(self) -> DiceRollDistribution:
        return DiceRollDistribution.from_pdf(
    ...
    # Distribution.outcomes
    # ---------------------
    def test_die_one_outcomes(self, die_one: DiceRollDistribution):
        assert die_one.outcomes == {
            DiceRoll.ONE, DiceRoll.TWO, DiceRoll.THREE,
            DiceRoll.FOUR, DiceRoll.FIVE, DiceRoll.SIX}
    
    def test_die_two_outcomes(self, die_two: DiceRollDistribution):
        assert die_two.outcomes == {
            DiceRoll.ONE, DiceRoll.TWO, DiceRoll.THREE,
            DiceRoll.FOUR, DiceRoll.FIVE, DiceRoll.SIX}
    
    def test_die_biased_outcomes(self, die_biased: DiceRollDistribution):
    ...
    # Distribution.probabilities
    # --------------------------
    def test_die_one_probabilities(self, die_one: DiceRollDistribution):
        assert die_one.probabilities == (1/6, 1/6, 1/6, 1/6, 1/6, 1/6)
    
    def test_die_two_probabilities(self, die_two: DiceRollDistribution):
        assert die_two.probabilities == (1/6, 1/6, 1/6, 1/6, 1/6, 1/6)
    
    def test_die_biased_probabilities(self, die_biased: DiceRollDistribution):
    ...
    def test_die_one_remove_one(
        self,
        die_one: DiceRollDistribution
    ):
        die_one.remove([DiceRoll.ONE])
        assert die_one == DiceRollDistribution([
    ...
    def test_die_one_bias(
        self,
        die_one: DiceRollDistribution,
        die_biased: DiceRollDistribution
    ):
    ...
    def test_dice_one_and_two_intersect(
        ...
        die_one: DiceRollDistribution,
        die_two: DiceRollDistribution):
        assert die_one.intersect(die_two) == Distribution[tuple[DiceRoll, DiceRoll]]([
    ...
    def test_dice_one_and_biased_intersect(
        ...
        assert (
            ...
            Distribution[tuple[DiceRoll, DiceRoll]].from_pdf({
    ...
    def test_die_one_intersect_self_once(
        ...
        truth_die = Distribution[tuple[DiceRoll]](
            outcomes=[
                (DiceRoll.ONE,), (DiceRoll.TWO,), (DiceRoll.THREE,),
                (DiceRoll.FOUR,), (DiceRoll.FIVE,), (DiceRoll.SIX,)
            ])
    ...
    def test_die_one_intersect_self_twice(
        ...
        truth_die = Distribution[tuple[DiceRoll, DiceRoll]]([
            (DiceRoll.ONE, DiceRoll.ONE), (DiceRoll.ONE, DiceRoll.TWO), (DiceRoll.ONE, DiceRoll.THREE), (DiceRoll.ONE, DiceRoll.FOUR), (DiceRoll.ONE, DiceRoll.FIVE), (DiceRoll.ONE, DiceRoll.SIX), 
            (DiceRoll.TWO, DiceRoll.ONE), (DiceRoll.TWO, DiceRoll.TWO), (DiceRoll.TWO, DiceRoll.THREE), (DiceRoll.TWO, DiceRoll.FOUR), (DiceRoll.TWO, DiceRoll.FIVE), (DiceRoll.TWO, DiceRoll.SIX), 
            (DiceRoll.THREE, DiceRoll.ONE), (DiceRoll.THREE, DiceRoll.TWO), (DiceRoll.THREE, DiceRoll.THREE), (DiceRoll.THREE, DiceRoll.FOUR), (DiceRoll.THREE, DiceRoll.FIVE), (DiceRoll.THREE, DiceRoll.SIX), 
            (DiceRoll.FOUR, DiceRoll.ONE), (DiceRoll.FOUR, DiceRoll.TWO), (DiceRoll.FOUR, DiceRoll.THREE), (DiceRoll.FOUR, DiceRoll.FOUR), (DiceRoll.FOUR, DiceRoll.FIVE), (DiceRoll.FOUR, DiceRoll.SIX), 
    ...
    def test_die_biased_intersect_self_twice(
        ...
        truth_die = Distribution[tuple[DiceRoll, DiceRoll]].from_pdf({
    ...
    # Distribution.sample()
    # ---------------------
    def test_die_one_sample_all(self, die_one: DiceRollDistribution):
        assert set(die_one.sample(n=6)) == die_one.outcomes
    
    def test_die_two_sample_all(self, die_two: DiceRollDistribution):
        assert set(die_two.sample(n=6)) == die_two.outcomes
    
    def test_die_biased_sample_all(self, die_biased: DiceRollDistribution):
        assert set(die_biased.sample(n=6)) == die_biased.outcomes

    # ----------------------------------
    # Distribution.sample_distribution()
    # ----------------------------------
    def test_die_one_sample_one_distribution(
        self,
        die_one: DiceRollDistribution
    ):
        die_one_sampled = die_one.sample_distribution(1)
        truth_die = Distribution[tuple[DiceRoll]](
            outcomes=[
                (DiceRoll.ONE,), (DiceRoll.TWO,), (DiceRoll.THREE,),
                (DiceRoll.FOUR,), (DiceRoll.FIVE,), (DiceRoll.SIX,)
    ...
    def test_die_two_sample_one_distribution(
        self,
        die_two: DiceRollDistribution
    ):
        die_one_sampled = die_two.sample_distribution(1)
        truth_die = Distribution[tuple[DiceRoll]](
            outcomes=[
                (DiceRoll.ONE,), (DiceRoll.TWO,), (DiceRoll.THREE,),
                (DiceRoll.FOUR,), (DiceRoll.FIVE,), (DiceRoll.SIX,)
    ...
    def test_die_biased_sample_one_distribution(
        self,
        die_biased: DiceRollDistribution
    ):
        die_biased_intersected = die_biased.sample_distribution(1)
        truth_die = Distribution[tuple[DiceRoll]](
            outcomes=[
                (DiceRoll.ONE,), (DiceRoll.TWO,), (DiceRoll.THREE,),
                (DiceRoll.FOUR,), (DiceRoll.FIVE,), (DiceRoll.SIX,)
    ...
    def test_die_one_sample_two_distributions_with_replacement(
        self,
        die_one: DiceRollDistribution
    ):
        die_one_sampled = die_one.sample_distribution(n=2, replace=True)
        truth_die = Distribution[tuple[DiceRoll, DiceRoll]]([
            (DiceRoll.ONE, DiceRoll.ONE), (DiceRoll.ONE, DiceRoll.TWO), (DiceRoll.ONE, DiceRoll.THREE), (DiceRoll.ONE, DiceRoll.FOUR), (DiceRoll.ONE, DiceRoll.FIVE), (DiceRoll.ONE, DiceRoll.SIX), 
            (DiceRoll.TWO, DiceRoll.ONE), (DiceRoll.TWO, DiceRoll.TWO), (DiceRoll.TWO, DiceRoll.THREE), (DiceRoll.TWO, DiceRoll.FOUR), (DiceRoll.TWO, DiceRoll.FIVE), (DiceRoll.TWO, DiceRoll.SIX), 
            (DiceRoll.THREE, DiceRoll.ONE), (DiceRoll.THREE, DiceRoll.TWO), (DiceRoll.THREE, DiceRoll.THREE), (DiceRoll.THREE, DiceRoll.FOUR), (DiceRoll.THREE, DiceRoll.FIVE), (DiceRoll.THREE, DiceRoll.SIX), 
    ...
    def test_die_two_sample_two_distributions_with_replacement(
        self,
        die_two: DiceRollDistribution
    ):
        die_two_sampled = die_two.sample_distribution(n=2, replace=True)
        truth_die = Distribution[tuple[DiceRoll, DiceRoll]]([
            (DiceRoll.ONE, DiceRoll.ONE), (DiceRoll.ONE, DiceRoll.TWO), (DiceRoll.ONE, DiceRoll.THREE), (DiceRoll.ONE, DiceRoll.FOUR), (DiceRoll.ONE, DiceRoll.FIVE), (DiceRoll.ONE, DiceRoll.SIX), 
            (DiceRoll.TWO, DiceRoll.ONE), (DiceRoll.TWO, DiceRoll.TWO), (DiceRoll.TWO, DiceRoll.THREE), (DiceRoll.TWO, DiceRoll.FOUR), (DiceRoll.TWO, DiceRoll.FIVE), (DiceRoll.TWO, DiceRoll.SIX), 
            (DiceRoll.THREE, DiceRoll.ONE), (DiceRoll.THREE, DiceRoll.TWO), (DiceRoll.THREE, DiceRoll.THREE), (DiceRoll.THREE, DiceRoll.FOUR), (DiceRoll.THREE, DiceRoll.FIVE), (DiceRoll.THREE, DiceRoll.SIX), 
    ...
    def test_die_biased_sample_two_distributions_with_replacement(
        self,
        die_biased: DiceRollDistribution
    ):
        die_biased_intersected = die_biased.sample_distribution(n=2, replace=True)
        truth_die = Distribution[tuple[DiceRoll, DiceRoll]].from_pdf({
            (DiceRoll.ONE, DiceRoll.ONE): 0.01, (DiceRoll.ONE, DiceRoll.TWO): 0.01, (DiceRoll.ONE, DiceRoll.THREE): 0.01, (DiceRoll.ONE, DiceRoll.FOUR): 0.01, (DiceRoll.ONE, DiceRoll.FIVE): 0.01, (DiceRoll.ONE, DiceRoll.SIX): 0.05, 
            (DiceRoll.TWO, DiceRoll.ONE): 0.01, (DiceRoll.TWO, DiceRoll.TWO): 0.01, (DiceRoll.TWO, DiceRoll.THREE): 0.01, (DiceRoll.TWO, DiceRoll.FOUR): 0.01, (DiceRoll.TWO, DiceRoll.FIVE): 0.01, (DiceRoll.TWO, DiceRoll.SIX): 0.05, 
            (DiceRoll.THREE, DiceRoll.ONE): 0.01, (DiceRoll.THREE, DiceRoll.TWO): 0.01, (DiceRoll.THREE, DiceRoll.THREE): 0.01, (DiceRoll.THREE, DiceRoll.FOUR): 0.01, (DiceRoll.THREE, DiceRoll.FIVE): 0.01, (DiceRoll.THREE, DiceRoll.SIX): 0.05, 
    ...
    def test_die_one_sample_two_distributions_without_replacement(
        self,
        die_one: DiceRollDistribution
    ):
        die_one_intersected = die_one.sample_distribution(n=2, replace=False)
        truth_die = Distribution[tuple[DiceRoll, DiceRoll]]([
    ...
    def test_die_two_sample_two_distributions_without_replacement(
        self,
        die_two: DiceRollDistribution
    ):
        die_two_intersected = die_two.sample_distribution(n=2, replace=False)
        truth_die = Distribution[tuple[DiceRoll, DiceRoll]]([
            (DiceRoll.ONE, DiceRoll.TWO), (DiceRoll.ONE, DiceRoll.THREE), (DiceRoll.ONE, DiceRoll.FOUR), (DiceRoll.ONE, DiceRoll.FIVE), (DiceRoll.ONE, DiceRoll.SIX), 
            (DiceRoll.TWO, DiceRoll.ONE), (DiceRoll.TWO, DiceRoll.THREE), (DiceRoll.TWO, DiceRoll.FOUR), (DiceRoll.TWO, DiceRoll.FIVE), (DiceRoll.TWO, DiceRoll.SIX), 
            (DiceRoll.THREE, DiceRoll.ONE), (DiceRoll.THREE, DiceRoll.TWO), (DiceRoll.THREE, DiceRoll.FOUR), (DiceRoll.THREE, DiceRoll.FIVE), (DiceRoll.THREE, DiceRoll.SIX), 
    ...
    def test_die_biased_sample_two_distributions_without_replacement(
        self,
        die_biased: DiceRollDistribution
    ):
        die_biased_intersected = die_biased.sample_distribution(n=2, replace=False)
        truth_die = Distribution[tuple[DiceRoll, DiceRoll]].from_pdf({
    ...
    def test_dice_explicitly_sorted(self):
        dice_rolls = Distribution[tuple[DiceRoll, DiceRoll, DiceRoll]]([
        ...
        assert (
            ...
            Distribution[tuple[DiceRoll, DiceRoll, DiceRoll]].from_pdf({
    ...
    def test_die_two_sample_distribution_without_replacement_and_sort(
        self,
        die_two: DiceRollDistribution):
        die_sampled = die_two.sample_distribution(n=2, replace=False)
        assert (
    ...
    def test_dice_explicitly_countered(self):
        dice_rolls = Distribution[tuple[DiceRoll, DiceRoll]]([
        ...
        assert (
            ...
            Distribution[HashableCounter[DiceRoll]].from_pdf({
    ...
    def test_die_one_sample_without_replacement_and_counter(
        self,
        die_one: DiceRollDistribution):
        die_sampled = die_one.sample_distribution(n=2, replace=False)
        assert (
            die_sampled.to_counter() == 
            Distribution[HashableCounter[DiceRoll]]([
    ...
    def test_die_one_filter_all_but_six(
        ...
        assert (
            ...
            DiceRollDistribution([DiceRoll.SIX])
        )
...
class TestBadDistributions:
    ...
    def test_instantiate_outcomes_wrong_type(self):
        # with pytest.raises(TypeError):
            distribution = Distribution[str]([0, 1])  # pyright: ignore[reportArgumentType]
    ...
    # --------------------------
    # Distribution.__delitem__()
    # --------------------------
    def test_delete_nonexistent_outcome(self):
        distribution = Distribution[int]([0])
        try:
            del distribution[1]
        except KeyError as e:
            assert str(e) == '"Outcome \'1\' not in distribution outcomes."'
            return
        assert False
    ...
    def test_delete_all_outcomes(self):
        ...
        try:
            ...
            assert str(e) == "Cannot delete the only outcome '0' in the distribution."
            return
        assert False
    ...
    def test_remove_nonexistent_outcome(self):
        ...
        try:
            ...
            assert str(e) == "'Subset of provided outcomes do not exist in distribution.'"
            return
        assert False
    
    def test_remove_nonexistent_outcome_superset(self):
        distribution = Distribution[int]([0, 1])
        try:
            distribution.remove([1, 2])
        except KeyError as e:
            assert str(e) == "'Subset of provided outcomes do not exist in distribution.'"
            return
        assert False
    ...
    def test_remove_duplicate_outcomes(self):
        distribution = Distribution[int]([0, 1])
        try:
            distribution.remove([1, 1])
        except ValueError as e:
            assert str(e) == "Provided outcomes contains duplicates."
            return
        assert False
    ...
    def test_rebalance_outcome_does_not_exist(self):
        distribution = Distribution[int]([0])
        try:
            distribution.rebalance(2, 0.5)
        except KeyError as e:
            assert str(e) == "\"Outcome '2' does not exist in this distribution.\""
            return
        assert False
    ...
    def test_sample_too_many(self):
        distribution = Distribution[int]([0, 1])
        try:
            distribution.sample(3, replace=False)
        except ValueError as e:
            assert str(e) == "Cannot sample 3 outcomes without replacement from an distribution with only 2 unique outcomes."
            return
        assert False
    ...
    # Distribution.sample_distribution()
    # ----------------------------------
    def test_sample_distribution_zero(self):
        distribution = Distribution[int]([0, 1])
        try:
            distribution.sample_distribution(0)
        except ValueError as e:
            assert str(e) == "n must be at least 1 to form a new distribution."
            return
        assert False
    
    def test_sample_distribution_negative(self):
        distribution = Distribution[int]([0, 1])
        try:
            distribution.sample_distribution(-1)
        except ValueError as e:
            assert str(e) == "n must be at least 1 to form a new distribution."
            return
        assert False
    
    def test_sample_distribution_too_many(self):
        distribution = Distribution[int]([0, 1])
        try:
            distribution.sample_distribution(3, replace=False)
        except ValueError as e:
            assert str(e) == "Cannot sample 3 outcomes without replacement from an distribution with only 2 unique outcomes."
            return
        assert False
    ...
    def test_unsortable(self):
        distribution = Distribution[int]([0, 1])
        with pytest.raises(TypeError):
            distribution.sorted()  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def test_uncounterable(self):
        distribution = Distribution[int]([0, 1])
        with pytest.raises(TypeError):
            distribution.to_counter()  # pyright: ignore[reportAttributeAccessIssue]

    # ---------------------
    # Distribution.filter()
    # ---------------------
    def test_filter_type_mismatch(self):
        distribution = Distribution[str](['a', 'b'])
        with pytest.raises(TypeError):
            distribution.filter(lambda x: x < 0)  # pyright: ignore[reportOperatorIssue]
    ...
    def test_filter_by_probability_type_mismatch(self):
        distribution = Distribution[str](['a', 'b'])
        with pytest.raises(TypeError):
            distribution.filter_by_probability(lambda x: x + 'a')  # pyright: ignore[reportOperatorIssue]
    ...
    def test_query_type_mismatch(self):
        distribution = Distribution[str](['a', 'b'])
        with pytest.raises(TypeError):
            distribution.query(lambda x: x < 0)  # pyright: ignore[reportOperatorIssue]
    ...
    def test_reduce_op_wrong_type(self):
        distribution = Distribution[int]([0, 1])
        with pytest.raises(TypeError):
            distribution.reduce(op.add)  # pyright: ignore[reportAttributeAccessIssue]
    
    def test_reduce_initial_wrong_type(self):
        distribution = Distribution[tuple[int, ...]]([(0, 1), (2, 1)])
        with pytest.raises(TypeError):
            distribution.reduce(op.add, 'a')  # pyright: ignore[reportArgumentType]
    ...
    def test_map_func_wrong_type(self):
        distribution = Distribution[str](['a', 'b'])
        with pytest.raises(TypeError):
            distribution.map(lambda x: x + 0)  # pyright: ignore[reportOperatorIssue]
